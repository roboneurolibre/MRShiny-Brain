...
**MRS thermometry:** MRS thermometry involves estimating the temperature of a brain region by measuring the chemical shift difference between the N-acetylaspartate (NAA) and water peaks. The chemical shift difference between the NAA peak and the water peak was calculated by subtracting the chemical shift value of NAA from that of water. This difference was then used in conjunction with a pre-established calibration curve that relates the chemical shift difference to temperature (REF Zhou 2016).
...
_Thermometry_
1.	Temperature: Temperature in Celcius (C) in each brain region of interest.
